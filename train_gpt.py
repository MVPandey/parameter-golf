...
def main() -> None:
    ...
    if args.energy_enabled and rank == 0:
        ...
        class EnergyNet(nn.Module):
            def __init__(s, d_model, vocab_size, d_energy):
                super().__init__()
                s.proj = nn.Sequential(nn.Linear(d_model, d_energy), nn.GELU(), nn.Linear(d_energy, d_energy))
                s.tok_emb = nn.Embedding(vocab_size, d_energy)
                s.log_temp = nn.Parameter(torch.tensor(0.0))
                for m in s.proj.modules():
                    if isinstance(m, nn.Linear): nn.init.xavier_uniform_(m.weight); nn.init.zeros_(m.bias)
                nn.init.normal_(s.tok_emb.weight, std=0.01)
            def forward(s, h):
        ...
        sl = effective_eval_seq_len
        n_energy = min(args.energy_tokens, val_tokens.numel() - 1)
        n_seqs = n_energy // sl
        eval_model.eval()
        with torch.inference_mode():
        ...
        handle.remove()
        all_h, all_logits, all_tgt = torch.cat(all_h), torch.cat(all_logits), torch.cat(all_tgt)
        log0(f"energy:collected {all_h.shape[0]} seqs")
        ...
        for step in range(n_steps):
            ...
            for g in eopt.param_groups: g['lr'] = args.energy_lr * 0.5 * (1 + math.cos(math.pi * prog))
            idx = torch.randint(0, n_train, (32,))
            h, ar, tgt = all_h[idx].to(device), all_logits[idx].to(device), all_tgt[idx].to(device)
            loss = F.cross_entropy((ar + enet(h)).reshape(-1, args.vocab_size), tgt.reshape(-1))
            eopt.zero_grad(); loss.backward()
            torch.nn.utils.clip_grad_norm_(enet.parameters(), 1.0); eopt.step()
        log0(f"energy:trained {n_steps} steps in {time.perf_counter()-t_e:.1f}s")
        ...
        with torch.inference_mode():
            for bs in range(0, total_seqs, 32):
                ...
                n = float(y.numel())
                loss_sum += bl.to(torch.float64) * n; token_count += n
        ...
        handle.remove()
        e_loss = (loss_sum / token_count).item()
        e_bpb = e_loss / math.log(2.0) * token_count.item() / byte_count.item()
        log0(f"energy_correction val_loss:{e_loss:.4f} val_bpb:{e_bpb:.4f}")
